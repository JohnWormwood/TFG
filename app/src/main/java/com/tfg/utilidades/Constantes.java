package com.tfg.utilidades;

import com.tfg.modelos.PrecioMejora;

import java.util.Arrays;
import java.util.List;

public final class Constantes {
    public static final int NIVEL_INICIAL = 1;

    public static class Aldea {
        public static final int POBLACION_INICIAL = 10;
        public static final int AUMENTO_MAX_POBLACION_POR_NIVEL = 10;
        public static final int NIVEL_MAXIMO = 10;
        public static final int COMIDA_INICIAL = 50;

        public static final List<PrecioMejora> PRECIOS_MEJORAS = Arrays.asList(
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0)
        );
    }

    public static class Edificio {
        public static final int NIVEL_MAXIMO = 10;
        public static final int AUMENTO_MAX_ALDEANOS_POR_NIVEL = 10;
    }

    public static class CabaniaCaza {
        public static final int PROBABILIDAD_MUERTE = 10;

        public static final List<PrecioMejora> PRECIOS_MEJORAS = Arrays.asList(
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0)
        );
    }
    public static class CasetaLeniador {

        public static final List<PrecioMejora> PRECIOS_MEJORAS = Arrays.asList(
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0)
        );
    }
    public static class Carpinteria {

        public static final List<PrecioMejora> PRECIOS_MEJORAS = Arrays.asList(
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0)
        );
    }

    public static class Granja {

        public static final List<PrecioMejora> PRECIOS_MEJORAS = Arrays.asList(
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0)
        );
    }

    public static class Mina {

        public static final List<PrecioMejora> PRECIOS_MEJORAS = Arrays.asList(
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0),
                new PrecioMejora(10, 0, 0, 0, 0, 0)
        );
    }

}
